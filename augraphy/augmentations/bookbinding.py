...
class BookBinding(Augmentation):
    ...
    def __init__(
        self,
        radius_range: Tuple[int, int] = (1, 100),
        curve_range: Tuple[int, int] = (200, 300),
        mirror_range: Tuple[float, float] = (0.2, 0.5),
        curling_direction: int = random.choice([0, 1]),
        p: float = 1,
    ):
        super().__init__(p=p)
    ...
    def add_book_shadow(self, img: np.ndarray, radius: int, angle: int) -> np.ndarray:
    ...
    def curve_page(self, img: np.ndarray, curve_range: Tuple[int, int]) -> np.ndarray:
    ...
    def __call__(self, image: np.ndarray, force: bool = False) -> np.ndarray:
        if force or self.should_run():
            ...
            if curve_down:
                ...
                image_shadow_right = self.add_book_shadow(
                    np.flipud(image_added_border_right),
                    radius,
                    angle,
                )
            ...
            if curve_down:
                ...
                image_shadow_left = np.fliplr(
                    self.add_book_shadow(np.fliplr(image_added_border_left), radius, angle),
                )
                image_left = self.curve_page(
                    np.fliplr(np.flipud(image_shadow_left)),
                    curve_range * 3,
                )
            else:
                page_border = PageBorder(
                ...
                image_shadow_left = np.fliplr(
                    self.add_book_shadow(image_added_border_left, radius, angle),
                )
            ...
            target_points = np.float32(
                [[curve_xsize, 0], [cxsize, 0], [cxsize, cysize], [curve_xsize, cysize]],
            )
            ...
            if not curve_down:
                image_left = np.fliplr(
                    self.add_book_shadow(np.fliplr(image_left), radius, angle),
                )
